...
public class HomePage implements ActionListener, KeyListener, ItemListener, FocusListener, ChangeListener {
    ...
    public void initFindBookPanel() {
        JLabel[] innerLabel = new JLabel[2];
        String[] labelName = {
                "Filters",
                "Department"
        };
        for (int i = 0 ; i < innerLabel.length ; i++) {
            innerLabel[i] = new JLabel(labelName[i]);
        }
        innerLabel[0].setBounds(30,30,100,40);
        innerLabel[0].setFont(new Font("Comic Sans MS", Font.BOLD, 30));
        Util.setLabelHighlighted(innerLabel[0], Color.YELLOW);
        tabbedPane[8].add(innerLabel[0]);
    }
